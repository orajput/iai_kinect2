...
private:
  ...
  const std::string topicColor, topicDepth, topicPxDepth;
...
public:
  Receiver(const std::string &topicColor, const std::string &topicDepth, const std::string &topicPxDepth, const bool useExact, const bool useCompressed)
    : topicColor(topicColor), topicDepth(topicDepth), topicPxDepth(topicPxDepth), useExact(useExact), useCompressed(useCompressed),
      updateImage(false), save(false), running(false), frame(0), queueSize(5),
      nh("~"), spinner(0), it(nh)
  {
    pxLoc = cv::Point(0,0);
...
private:
  void start()
  {
    ...
    pxDepthPub = nh.advertise<std_msgs::UInt16>(topicPxDepth, queueSize);
  ...
  void imageViewer()
  {
    ...
    cv::setMouseCallback("Image Viewer", Receiver::mouseCallbackStatic, this);
    oss << "starting...";
    std_msgs::UInt16 msg;
    ...
    for(; running && ros::ok();)
    {
      if(updateImage)
      {
        ...
        if(pxDepthPub.getNumSubscribers() > 0)
        {
          const uint16_t *itD = depth.ptr<uint16_t>(pxLoc.y);
          itD += pxLoc.x;
          msg.data = *itD;
          pxDepthPub.publish(msg);
        }
      }
  ...
  static void mouseCallbackStatic( int event, int x, int y, int flags, void* that )
  {
      Receiver* recv = reinterpret_cast<Receiver*>(that);
      recv->mouseCallback(event, x, y, flags);
  }
...
void help(const std::string &path)
{
  std::cout << path << FG_BLUE " [options]" << std::endl
            ...
            << FG_GREEN "  visualization" NO_COLOR ": " FG_YELLOW "'image'" << std::endl
...
int main(int argc, char **argv)
{
  ...
  ros::init(argc, argv, "kinect2_plotpx", ros::init_options::AnonymousName);
  ...
  std::string topicPxDepth = "/pxDepth";
  bool useExact = true;
  ...
  topicPxDepth = "/" + ns + topicPxDepth;
  OUT_INFO("topic color: " FG_CYAN << topicColor << NO_COLOR);
  OUT_INFO("topic depth: " FG_CYAN << topicDepth << NO_COLOR);

  Receiver receiver(topicColor, topicDepth, topicPxDepth, useExact, useCompressed);
